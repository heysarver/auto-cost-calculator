...
def calculate_loan_cost(loan_amount, interest_rate, loan_length, down_payment, annual_maintenance_cost, full_insurance_cost):
    monthly_interest_rate = interest_rate / 12 / 100
    loan_term_in_months = loan_length * 12
    monthly_payment = loan_amount * (monthly_interest_rate * (1 + monthly_interest_rate)**loan_term_in_months) / ((1 + monthly_interest_rate)**loan_term_in_months - 1)

    total_loan_cost = monthly_payment * loan_term_in_months + down_payment
    total_maintenance_cost = annual_maintenance_cost * loan_length
    total_insurance_cost = full_insurance_cost * loan_length
...
def get_average_interest_rate(api_key, series_id):
    ...
    interest_rates = [float(obs["value"]) for obs in observations if obs["value"] != "."]
    return sum(interest_rates) / len(interest_rates)
